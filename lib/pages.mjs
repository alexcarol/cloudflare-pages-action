...
export async function syncPages(client, accountId, config, allowRecreate) {
  ...
  // Fetch the project to get the actual subdomain from Cloudflare's API response
  const project = await client.pages.projects.get(config.name, {
    account_id: accountId,
  });

  // Use the subdomain from the API response
  if (!project.subdomain) {
    console.error('\nWARNING: Pages project was configured successfully, but the subdomain field was not returned by the Cloudflare API.');
    console.error('Unable to determine the Pages URL. Please check the Cloudflare dashboard.');
    console.error(`Dashboard: https://dash.cloudflare.com/${accountId}/pages/view/${config.name}`);
    throw new Error('Cloudflare API did not return subdomain field for Pages project');
  }
