 ...
 */
export async function syncWorker(client, accountId, config, secrets, workingDir) {
  ...
  try {
    ...
    if (!subdomainInfo.subdomain) {
      console.error('\nWARNING: Worker was deployed successfully, but the subdomain field was not returned by the Cloudflare API.');
      console.error('Unable to determine the Worker URL. Please check the Cloudflare dashboard.');
      throw new Error('Cloudflare API did not return subdomain field for Workers');
    }
